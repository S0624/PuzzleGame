...
public class NetMatchGameManager : MonoBehaviourPunCallbacks
{
    ...
    private void TestUpdate()
    {
        ...
        // キューブの移動処理.
            if (PhotonNetwork.LocalPlayer.ActorNumber == player.ActorNumber)
            {
                if (_obstaclePrev[add] != _fieldData[add].GetObstacleNum())
                {
                    ...
                    if (_addObstacle[add] == 0)
                    {
                        ...
                        //if (_isStop)
                        {
                            _addObstacle[add] += _obstacle[add];
                            //Debug.Log(_obstacle[add] + "だだ" + _addObstacle[add]);
                        }
                        //else
                        {
    ...
    private void ObstacleCalculation(int add)
    {
        ...
        //Debug.Log(_addObstacle[0] + "あ" + _addObstacle[1] + "い" + testNum);
        //if(testNum < 0)
        //{
        //    _addObstacle[0] = 0;
        ...
        //    Debug.Log("こんにちわ");
        //}
        for (int i = 0; i < _addObstacle.Length; i++)
        {
            _prevObstacleAdd[i] = _addObstacle[i];
        }
